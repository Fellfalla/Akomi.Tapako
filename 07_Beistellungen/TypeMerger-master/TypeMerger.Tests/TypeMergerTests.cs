...
namespace TypeMerger.Tests {
    ...
    public class TypeMergerTests {
        ...
        [TestMethod]
        public void TestReferenceCopying()
        {
            int value = 1;
            var referenceClass1 = new ReferenceTestClass() {Value = value };
            var strategy1 = new StrategyClass() {ParentClass = referenceClass1};
            referenceClass1.Strategy = strategy1;
            var result1 = Merger.Merge(new object(), referenceClass1);

            int value2 = 2;
            //result1.GetType().GetProperty("Value").SetValue(result1,value2);

            Console.WriteLine(GetAddress(result1));
            Assert.AreEqual(value2,result1.GetType().GetProperty("Strategy").PropertyType.GetMethod("ReturnReferencedValue").Invoke(result1.GetType().GetProperty("Strategy").GetValue(result1),new object[]{}));
        ...
        public static string GetAddress(object o)
{
    ...
    else
    {
        unsafe
        {
            System.TypedReference tr = new TypedReference();
            TypedReference.SetTypedReference(tr, o);
    ...
    {
        public ReferenceTestClass ParentClass { get; set; }
